 ...
 * @param {Function} opts.onItemProgress - optional (idx, total, title) => void, called before each item
 * @returns {Promise<{ filePath: string, ext: string }>}
 */
export async function downloadZip({ items, ffmpegDir, outDir, basename, zipName, onItemProgress }) {
  ...
  for (const item of items) {
    idx++;
    if (onItemProgress) {
      try { onItemProgress(idx - 1, items.length, item.title); } catch {}
    }
    try {
