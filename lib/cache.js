 ...
 * Keeps the most recent entries, deletes the oldest.
 */
const MAX_CACHE_ENTRIES = 50000;
const MAX_CACHE_SIZE = 100 * 1024 * 1024; // 100 MB
export function cacheEnforceLimit() {
  const row = stmtCount.get();
  if (row.cnt > MAX_CACHE_ENTRIES || row.totalSize > MAX_CACHE_SIZE) {
    // Delete oldest entries (rowid auto-increments, so lower = older)
    const targetEntries = Math.floor(MAX_CACHE_ENTRIES * 0.8);
    const deleteCount = row.cnt - targetEntries;
    if (deleteCount > 0) {
      const info = db.prepare(
        `DELETE FROM cache WHERE rowid IN (SELECT rowid FROM cache ORDER BY rowid ASC LIMIT ?)`
      ).run(deleteCount);
      const remaining = stmtCount.get();
      console.log(`  ▸ cache: pruned ${info.changes} oldest entries (${remaining.cnt} remaining)`);
    }
  }
}

// --- graceful shutdown (server.js handles SIGINT/SIGTERM → exit → this closes DB) ---
